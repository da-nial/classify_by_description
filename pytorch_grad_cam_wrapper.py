...
def hilacam(image, text, model, device, index=None, cam_size=None):
    ...
    R = torch.eye(num_tokens, num_tokens,
                  dtype=image_attn_blocks[0].attn_probs.dtype)  # * change to cpu to resolve memory overflow
    # weighted activation
...
class CAMWrapper(object):
    CAM_LIST = ['gradcam', 'scorecam', 'gradcam++', 'ablationcam', 'xgradcam', 'eigencam', 'eigengradcam', 'layercam',
                'hilacam', 'groupcam', 'sscam1', 'sscam2', 'rawcam', 'testhila', 'gradientcam', 'gscorecam',
                'vitgradcam', 'rise', 'gscorecambeta', 'testcam']
    ...
    def _load_cam(self):
        if self.version == 'hilacam':
            ...
            self.cam = self.CAM_DICT[self.version](model=self.model, target_layers=self.target_layers, use_cuda=True,
                                                   clip=self.is_clip, reshape_transform=self.cam_trans, hila=True)
            ...
            self.cam = self.CAM_DICT[self.version](self.model, self.target_layers[0], cluster_method='k_means',
            ...
            self.cam = self.CAM_DICT[self.version](model=self.model, image_size=img_size, mask_path=mask_path,
                                                   batch_size=64)
        else:
            self.cam = self.CAM_DICT[self.version](model=self.model, target_layers=self.target_layers,
                                                   use_cuda=True, is_clip=self.is_clip,
                                                   reshape_transform=self.cam_trans, is_transformer=self.is_transformer)
    ...
    def getCAM(self, input_img, input_text, cam_size, target_category):
        ...
        if self.version == 'hilacam':
            grayscale_cam = self.cam(
                input_img, input_text, self.model, 'cuda', cam_size=cam_size, index=target_category
            )
        elif self.version == 'groupcam':
            ...
            grayscale_cam = self.cam(
                input_img, input_text, class_idx=target_category, param_n=35, mean=0, sigma=2, mute=self.mute
            )
        elif self.version == 'layercam':
    ...
    def __call__(self, inputs, label, heatmap_size, is_tokenized=True):
