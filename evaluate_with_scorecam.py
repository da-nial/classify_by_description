...
from utils import pretty_print, get_metrics, display_cams
...
def evaluate(
    ...
    cam_wrapper = CAMWrapper(
        ...
        cam_version=hparams['cam_version'],
        preprocess=None,
        # target_category=None,  # TODO play with this
        is_clip=True,
        cam_trans=None,
        # is_transformer=False,  # TODO make this automated,
        drop=False,  # Whether to drop the channels in scorecam
        channels={},
        topk=300,  # Number of channels used by scorecam (with drop) or gscorecam,
        batch_size=hparams['batch_size'],
        mute=True  # disables tqdm logs
    )
    ...
    for batch_number, batch in tqdm(enumerate(dataloader)):
        ...
        img_np = reverse_transform(images[0])
        for class_label in class_labels:
            cam = cam_wrapper(
                inputs=(images, class_label),  # TODO fix this
                label=None,
            ...
            display_cams(img=img_np, cams=[cam])
            image_encodings = cam_wrapper.outputs  # TODO use this if possible
