...
class GScoreCAM(BaseCAM):
    def __init__(self, model, target_layers, use_cuda=True, reshape_transform=None, is_clip=False, drop=False,
                 mute=True, topk=None, channels=None, batch_size: int = 128, is_transformer: bool = False):
        super(GScoreCAM, self).__init__(model, target_layers, use_cuda,
                                        reshape_transform=reshape_transform, is_clip=is_clip, drop=drop, mute=mute,
                                        batch_size=batch_size, is_transformer=is_transformer)
        self.topk = topk
        self.use_bot = False
        self.drop = drop
        self.mute = mute
